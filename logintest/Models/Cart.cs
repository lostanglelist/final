...
namespace logintest.Models
{
    public class Cart
    {
        ...
        public CartItem GetCartItem(ImageModel image)
        {
            ...
            ci.image.ProductId == image.ProductId && ci.VGAId == Id);
        }
        ...
        {
            var cartItems = _context.CartItems.Where(ci => ci.VGAId == Id);
        ...
        {
            return CartItems ??
                (CartItems = _context.CartItems.Where(ci => ci.VGAId == Id).Include(ci => ci.image).ToList());
        }

        public float GetCartTotal()
        {
            return _context.CartItems.Where(ci => ci.VGAId == Id).Select(ci => ci.image.ProductPrice * ci.Quantity).Sum();
        }
    }
}